...
function assignUnifiedStatus(agents: MonthlyAgent[]): (MonthlyAgent & { status: AgentStatus })[] {
  const sorted = [...agents].sort((a, b) => b.roli - a.roli);
  const len = sorted.length;
  return sorted.map((a, i) => ({
    ...a,
    status: i < Math.max(3, Math.round(len * 0.15)) && a.closeRate >= GATE_THRESHOLDS.MIN_CR_FOR_PROMOTION
      ? "PROMOTE"
      : i >= len - Math.max(3, Math.round(len * 0.15))
        ? "EXIT RISK"
        : "HOLD",
  }));
}
...
export default function MonthlyStackRank() {
  const { monthlyT1, monthlyT2, monthlyT3, windowStart, windowEnd, workingDays } = useData();
  ...
  const allAgents = [...monthlyT1, ...monthlyT2, ...monthlyT3];
  const agents = assignUnifiedStatus(allAgents);

  const totalPremium = agents.reduce((s, a) => s + a.totalPremium, 0);
  const totalProfit = agents.reduce((s, a) => s + a.profit, 0);
  const avgROLI = agents.length ? agents.reduce((s, a) => s + a.roli, 0) / agents.length : 0;
  const topCount = agents.filter(a => a.status === "PROMOTE").length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <MetricCard label="Total Profit" value={formatCurrency(totalProfit)} color="green" />
        <MetricCard label="Avg ROLI" value={`${avgROLI.toFixed(2)}x`} />
        <MetricCard label="Top Performers" value={topCount} color="green" subtext={`of ${agents.length} agents`} />
      </div>
          ...
          <tbody>
            {agents.map((agent, i) => (
              ...
              >
                <td className="px-3 py-2.5 font-mono text-muted-foreground tabular-nums font-bold">{i + 1}</td>
                <td className="px-3 py-2.5 font-semibold text-foreground">
                  <button onClick={() => handleAgentClick(agent)} className="hover:text-blue-400 hover:underline transition-colors text-left">{agent.name}</button>
                </td>
