...
function ProfileHeader({
  name, tier, site, manager, startDate, endDate, flags, onPrint,
    ...
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 print:flex-row">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Link href="/leads-pool" className="text-muted-foreground hover:text-foreground print:hidden">
          ...
          <span className="text-xs font-mono text-muted-foreground">{site}</span>
        </div>
      ...
      <div className="hidden print:block text-xs font-mono text-muted-foreground">
        {startDate} — {endDate}
      </div>
    </div>
  );
...
function ComplianceScorecard({ days }: { days: ProfileDay[] }) {
  ...
  const avgGatesPassed = days.length > 0
    ? days.reduce((s, d) => s + d.gatesPassed, 0) / days.length
    : 0;
    ...
    <div className="bg-card border border-border rounded-md p-4 space-y-4 print:break-inside-avoid">
      <div className="flex items-center justify-between">
        <h2 className="text-xs font-mono uppercase tracking-widest text-muted-foreground flex items-center gap-2">
          <Shield className="h-3.5 w-3.5" /> 7-Gate Compliance (Latest Day: {latest.date})
        </h2>
        <div className="flex items-center gap-2">
          {latest.compliant
              ...
              <tbody>
                {[...days].reverse().map(d => (
                  <tr key={d.date} className="border-t border-border/20">
                    ...
                    <td className={cn("px-1 py-0.5 text-center font-bold",
                      d.compliant ? "text-emerald-400" : "text-red-400"
                    )}>{d.gatesPassed}</td>
                  </tr>
                ))}
              </tbody>
...
function ActivityBalance({ days, summary }: { days: ProfileDay[]; summary: NonNullable<ReturnType<typeof useAgentProfile>["summary"]> }) {
  ...
  return (
    <div className="bg-card border border-border rounded-md p-4 space-y-4 print:break-inside-avoid">
      ...
      {/* Pool/Pipeline ratio bar */}
      <div className="space-y-1">
        <div className="flex items-center justify-between text-[10px] font-mono">
          <span className="text-muted-foreground">Pipeline {(100 - poolPct).toFixed(0)}%</span>
          <span className={cn("font-bold", inTarget ? "text-emerald-400" : "text-amber-400")}>
            Pool {poolPct.toFixed(0)}%
          </span>
        </div>
        <div className="relative h-4 bg-border/30 rounded-full overflow-hidden">
          ...
          {/* Pipeline fill */}
          <div className="absolute left-0 h-full bg-blue-500/40 rounded-l-full"
            style={{ width: `${100 - poolPct}%` }} />
          {/* Pool fill */}
          <div className={cn("absolute right-0 h-full rounded-r-full", inTarget ? "bg-emerald-500/40" : "bg-amber-500/40")}
            style={{ width: `${poolPct}%` }} />
        </div>
        <div className="text-[9px] font-mono text-muted-foreground/50 text-center">
          Target band: {T3_POOL_KPI.MIN_POOL_PCT}–{T3_POOL_KPI.MAX_POOL_PCT}% pool
        </div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <MetricCard label="Combined Dials/Day" value={Math.round(summary.avgCombinedDials)}
          color={summary.avgCombinedDials >= T3_POOL_KPI.MIN_COMBINED_DIALS ? "green" : "red"}
          subtext={`${summary.totalCombinedDials} total`} />
        <MetricCard label="Pool % Avg" value={`${poolPct.toFixed(0)}%`}
          color={inTarget ? "green" : "amber"}
        ...
        <MetricCard label="Talk Time/Day" value={`${Math.round(summary.avgTalkTime)}m`}
          color={summary.avgTalkTime >= T3_POOL_KPI.MIN_TALK_TIME ? "green" : "red"}
          subtext={`${Math.round(summary.totalCombinedTalk)} min total`} />
        <MetricCard label="Long Calls/Day" value={summary.avgLongCalls.toFixed(1)}
          color={summary.avgLongCalls >= T3_POOL_KPI.MIN_LONG_CALLS ? "green" : "red"}
          subtext={`${summary.totalPoolLongCalls} total`} />
      </div>

      {/* Queue discipline */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <MetricCard label="Avg Queue Size" value={Math.round(summary.avgCallQueue)}
          color={summary.avgCallQueue <= T3_POOL_KPI.MAX_QUEUE ? "green" : "amber"}
          subtext={<span className="inline-flex items-center gap-1">Trend: <TrendIcon trend={summary.queueTrend} /> {summary.queueTrend}</span>} />
        <MetricCard label="Avg Past Due" value={Math.round(summary.avgPastDue)}
          ...
          subtext={<span className="inline-flex items-center gap-1">Trend: <TrendIcon trend={summary.pastDueTrend} /> {summary.pastDueTrend}</span>} />
        ...
        <MetricCard label="Compliance Rate" value={`${summary.complianceRate.toFixed(0)}%`}
          color={summary.complianceRate >= 80 ? "green" : summary.complianceRate >= 50 ? "amber" : "red"}
          subtext={`${summary.compliantDays}/${summary.days} days compliant`} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-card border border-border rounded-md p-4 space-y-4 print:break-inside-avoid">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <MetricCard label="Total Sales" value={summary.totalSales}
          color={summary.totalSales > 0 ? "green" : "red"} subtext={`${summary.avgSalesPerDay.toFixed(1)}/day`} />
        <MetricCard label="Total Premium" value={fmt(summary.totalPremium)}
          color="blue" subtext={`${fmt(summary.avgPremiumPerDay)}/day`} />
        <MetricCard label="Close Rate" value={`${summary.closeRate.toFixed(1)}%`}
          color={summary.closeRate >= 8 ? "green" : summary.closeRate >= 4 ? "amber" : "red"} />
        <MetricCard label="Pool Close Rate" value={`${summary.poolCloseRate.toFixed(1)}%`}
          color={summary.poolCloseRate >= 8 ? "green" : summary.poolCloseRate >= 4 ? "amber" : "red"} />
        <MetricCard label="IB Sales" value={summary.totalIbSales} subtext="Inbound" />
        <MetricCard label="OB + Pool Sales" value={summary.totalObSales + summary.totalPoolSales}
          subtext={`${summary.totalObSales} OB · ${summary.totalPoolSales} pool`} />
      </div>
          ...
          <table className="w-full text-[11px] font-mono">
            <thead>
              <tr className="border-b border-border text-muted-foreground">
                ...
                <th className="px-2 py-1 text-right">Reg Dials</th>
                <th className="px-2 py-1 text-right">Pool Dials</th>
                <th className="px-2 py-1 text-right">Pool %</th>
                <th className="px-2 py-1 text-right">Talk</th>
                ...
                <th className="px-2 py-1 text-right">Past Due</th>
                <th className="px-2 py-1 text-right">Queue</th>
                <th className="px-2 py-1 text-center">Gates</th>
              </tr>
            </thead>
            <tbody>
              {[...days].reverse().map(d => (
                <tr key={d.date} className="border-b border-border/20 hover:bg-accent/20">
                  ...
                  <td className={cn("px-2 py-1 text-right tabular-nums font-bold",
                    d.combinedSales > 0 ? "text-emerald-400" : ""
                  )}>{d.combinedSales}</td>
                  <td className="px-2 py-1 text-right tabular-nums">{d.combinedPremium > 0 ? fmt(d.combinedPremium) : "--"}</td>
                  <td className={cn("px-2 py-1 text-right tabular-nums",
                    (d.pastDue ?? 0) > 0 ? "text-red-400" : "text-emerald-400"
                  )}>{d.pastDue ?? "--"}</td>
                  <td className={cn("px-2 py-1 text-right tabular-nums",
                    (d.callQueue ?? 0) > T3_POOL_KPI.MAX_QUEUE ? "text-amber-400" : ""
                  )}>{d.callQueue ?? "--"}</td>
                  <td className="px-2 py-1 text-center">
                    {d.compliant
                      ? <span className="text-emerald-400 font-bold">{d.gatesPassed}/{T3_POOL_KPI.TOTAL_GATES}</span>
...
function PipelineHealth({ days, summary }: { days: ProfileDay[]; summary: NonNullable<ReturnType<typeof useAgentProfile>["summary"]> }) {
  ...
  const fuCompliance = (latest.pastDue != null && latest.todaysFollowUps != null)
    ? ((latest.pastDue + latest.todaysFollowUps) > 0 ? (1 - latest.pastDue / (latest.pastDue + latest.todaysFollowUps)) * 100 : 100)
    : null;

  return (
    <div className="bg-card border border-border rounded-md p-4 space-y-4 print:break-inside-avoid">
      <h2 className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Pipeline State (Latest: {latest.date})</h2>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <MetricCard label="Past Due" value={latest.pastDue ?? 0}
          color={(latest.pastDue ?? 0) === 0 ? "green" : "red"}
          subtext={<span className="inline-flex items-center gap-1"><TrendIcon trend={summary.pastDueTrend} /> {summary.pastDueTrend}</span>} />
        <MetricCard label="Call Queue" value={latest.callQueue ?? 0}
          color={(latest.callQueue ?? 0) <= T3_POOL_KPI.MAX_QUEUE ? "green" : "amber"}
          ...
          <table className="w-full text-[11px] font-mono">
            <thead>
              <tr className="border-b border-border text-muted-foreground">
                ...
                <th className="px-2 py-1 text-right">Past Due</th>
                <th className="px-2 py-1 text-right">Queue</th>
                <th className="px-2 py-1 text-right">New Leads</th>
                <th className="px-2 py-1 text-right">Today's F/U</th>
                <th className="px-2 py-1 text-right">Post-Sale</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={d.date} className="border-b border-border/20">
                    ...
                    <td className="px-2 py-1 text-right tabular-nums">
                      ...
                      {pdDelta != null && pdDelta !== 0 && (
                        <span className={cn("ml-1 text-[9px]", pdDelta < 0 ? "text-emerald-400" : "text-red-400")}>
                          {pdDelta > 0 ? "+" : ""}{pdDelta}
                        </span>
                      )}
                    </td>
                    <td className="px-2 py-1 text-right tabular-nums">
                      ...
                      {qDelta != null && qDelta !== 0 && (
                        <span className={cn("ml-1 text-[9px]", qDelta < 0 ? "text-emerald-400" : "text-red-400")}>
                          {qDelta > 0 ? "+" : ""}{qDelta}
                        </span>
                      )}
                    </td>
...
function CoachingSummary({ signals }: { signals: CoachingSignal[] }) {
  ...
  return (
    <div className="bg-card border border-border rounded-md p-4 space-y-4 print:break-inside-avoid">
      <h2 className="text-xs font-mono uppercase tracking-widest text-muted-foreground flex items-center gap-2">
        <Lightbulb className="h-3.5 w-3.5" /> Coaching Summary
      </h2>
        ...
        <div className="space-y-2">
          ...
          {strengths.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {strengths.map((s, i) => (
                <div key={i} className="bg-emerald-500/5 border border-emerald-500/20 rounded p-2">
                  <div className="text-[11px] font-mono font-bold text-emerald-400">{s.label}</div>
            ...
            <div className="space-y-1.5">
              {improvements.map((s, i) => (
                <div key={i} className={cn(
                  "border rounded p-2",
                  s.severity === "critical" ? "bg-red-500/5 border-red-500/20" : "bg-amber-500/5 border-amber-500/20"
                )}>
...
export default function AgentProfile() {
  ...
  return (
    <div className="space-y-4 print:space-y-3 max-w-[1200px] mx-auto">
      <ProfileHeader
        name={agentName}
        tier={agent?.tier ?? "T3"}
        site={agent?.site ?? "--"}
      ...
      {loading ? (
        ...
        <>
          ...
          <ActivityBalance days={days} summary={summary} />
